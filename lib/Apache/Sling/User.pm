...
sub add {
    ...
    my $res = Apache::Sling::Request::request( \$user,
        Apache::Sling::UserUtil::add_setup( $user->{ 'BaseURL' }, $actOnUser, $actOnPass, $properties ) );
    ...
    if ( open my ($input), "<", $file ) {
        while ( <$input> ) {
		    ...
		    if ( $column_headings[0] !~ /^[Uu][Ss][Ee][Rr]$/x ) {
		        croak "First CSV column must be the user ID, column heading must be \"user\". Found: \"" . $column_headings[0] . "\".\n";
		    }
		    if ( $column_headings[1] !~ /^[Pp][Aa][Ss][Ss][Ww][Oo][Rr][Dd]$/x ) {
		        croak "Second CSV column must be the user password, column heading must be \"password\". Found: \"" . $column_headings[0] . "\".\n";
		    }
...
sub change_password {
    ...
    my $res = Apache::Sling::Request::request( \$user,
        Apache::Sling::UserUtil::change_password_setup( $user->{ 'BaseURL' }, $actOnUser, $actOnPass, $newPass, $newPassConfirm ) );
...
sub update {
    ...
    my $res = Apache::Sling::Request::request( \$user,
        Apache::Sling::UserUtil::update_setup( $user->{ 'BaseURL' }, $actOnUser, $properties ) );
